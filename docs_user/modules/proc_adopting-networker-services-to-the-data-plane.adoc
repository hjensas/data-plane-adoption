...
. Cleanup any {networking_first_ref} agents that are no longer running.
+
In some cases there would be agents from the old dataplane which has been replaced or retired. The function these agents provided may be provided by a new agent running in {rhos_long}, or the agents function may have been replaced by other components. For example, DHCP Agents may no longer be needed, since OVN DHCP in {rhos_long} can provide this function.

.. List the agents. The list of agents varies depending on the services you had enabled:
+
----
$ oc exec openstackclient -- openstack network agent list
+--------------------------------------+------------------------------+--------------------------+-------------------+-------+-------+----------------------------+
...
.. For any agent in the list with `XXX` in the `Alive` field verify the Host and Agent Type, if the functions of this agent is no longer required and the agent has been permanently stopped on the {rhos_prev_long} host. Delete the agent by running:
+
----
